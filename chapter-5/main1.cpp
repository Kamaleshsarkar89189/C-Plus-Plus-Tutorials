...
int main(){
    ...
    std::cout << std::endl;
    std::cout << "Declaring pointer and assignment address : " << std::endl;
    ...
    int number1 {12}; 
    p_number = &number1;//Make it point to a valid address
    ...
    *p_number2 = 55; //Writing into junk address : BAD!
    ...
    p_number4 = new int;  //Dynamically allocate space for a single int on the heap
                          //This memory belongs to our program from now on. The system can;t use it for anything else, untill we return it.
                          //After this line executes, we will have a valid memory location allocated.
                          //The size of the allocated memory will be such that it can store the type pointer to by the pointer
    ...
    */
    ...
    //It is aslo possible to initialize the pointer with a valid
    //address up on declaration. Not with a nullptr
    int *p_number5{ new int}; //Memory location contains junk value
    int *p_number6{ new int (20)}; //use direct intitialization
    int *p_number7{ new int {23}}; //Use uniform inititalization
   ...
   std::cout << "*p_number5 : " << *p_number5 <<std::endl;
   delete p_number5;
   delete p_number5;//Ending problem ...

#include<iostream>
#include<iomanip>

int main(){
    float number1 {1.234567890789f}; //Precision : 7
    double number2 {1.23456768768980}; // Precision : 15
    long double number3 {1.12345678903456L}; //

    // Print out the sizes
    std::cout <<"sizeof float : " << sizeof(float) << std::endl;
    std::cout <<"sizeof double : " << sizeof(double) << std::endl;
    std::cout <<"sizeof long double : " << sizeof(long double) << std::endl;


    //Precison
    std::cout << std::setprecision(20);  //Control the precision from std::cout.
    std::cout << "number1 is : " << number1 << std::endl; // 7 digits
    std::cout << "number2 is : " << number2 << std::endl; // 15 digits
    std::cout << "number3 is : " << number3 << std::endl; // 15+ digits


    //Floating problems : The precision is usually too limited
    // For a loto of applications
    float number4 {1922400023.0f}; //Error : narrowing conversion
    // double number5 {1922400023.0}; 

    std::cout << "number4 : " <<number4 << std::endl;
    // std::cout << "number5 : " <<number5 << std::endl;

    //Scintific notation
    //What we have seen so far in terms of floating point types
    // is fixed notation. There is another notation, scientific that
    //is handly if you have really huge number or small numbers to represent

    std::cout << "--------------------------" <<std::endl;

    double number5 {192400023};
    double number6 {1.92400023e8};  //Can ommit the lower 00023
    double number7 {1.924e8};       //for simplicity if our application allows
                                
    double number8 {0.000000000003498};
    double number9 {3.498e-11}; //multiply with 10 exp(-11)

    std::cout << "Number5 is : " <<number5 << std::endl;
    std::cout << "Number6 is : " <<number6 << std::endl;
    std::cout << "Number7 is : " <<number7 << std::endl;
    std::cout << "Number8 is : " <<number8 << std::endl;
    std::cout << "Number9 is : " <<number9 << std::endl;


    //Infinity and NaN
    std::cout << std::endl;
    std::cout << "Infinity and NaN" << std::endl;

    double number10{-5.6}; // If the number is positive then + inf or negative inf
    double number11{};  //Initialize to 0  
    double number12{};  //Initialize to 0

    //Infinity
    double result {number10 / number11};

    std::cout << number10 << "/" << number11 << "yields" << result <<std::endl;
    std::cout << result << "+" << number10 << "yields" << result <<std::endl;

    //NaN
    result = number11 / number12;
    std::cout << number11 << "/" <<number12 << "=" <<result <<std::endl;

    return 0;
}